...
onMounted(() => {
  ...
  const dataV = test.value.map((o) => o.count);
  const img1 = new Image(30, 30);
  ...
  const barAvatar = {
    ...
    beforeDraw(chart, args, options) {
      const {
        ctx,
        chartArea: { top, bottom, left, right, widht, height },
        scales: { x, y }
      } = chart;

      ctx.save();

      console.log('x', x, x.getPixelForValue(0), y.getPixelForValue(dataV[0]));
      ctx.drawImage(img1, x.getPixelForValue(0) - 15, y.getPixelForValue(dataV[0]), 30, 30);
    }
  };
  if (chartDOM) {
    // @ts-ignore
...
<template>
  <Modal>
    ...
    >
      ...
      <div>
        <p class="font-light font-noto-sans-TC text-[#ef7ca4] text-[26px] lg:text-[34px]">目前比數</p>
      </div>
